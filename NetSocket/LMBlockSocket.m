...
- (instancetype)init
{
    if(self  = [super init])
    {
        ...
        ioctl(self.socket_,FIONBIO,&flag);
...
- (BOOL)acceptOnPort:(uint32_t)port error:(NSError *__autoreleasing *)error
{
    ...
    dispatch_async(self.delegateQueue, ^{
        while (1)
        {
            ...
            LMBlockSocket *socket = [[LMBlockSocket alloc]initWithSocket:client_sock];
...
- (void)receiveData
{
    ...
    dispatch_async(self.delegateQueue, ^{
        ...
        while (1)
        {
            ...
            if(len <= 0)
            {
//                NSLog(@"len:%zd--接收数据出错%d",len,errno);
                //系统信号导致没有收到消息
